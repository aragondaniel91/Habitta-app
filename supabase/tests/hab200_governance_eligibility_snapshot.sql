...
values
  (
    ...
    'authenticated', 'authenticated', 'hab200-admin@test.local', 'x',
    '{"full_name":"HAB 200 Admin"}'::jsonb, now(), now()
  ),
  (
    '20000000-0000-0000-0000-000000000011',
    '00000000-0000-0000-0000-000000000000',
    'authenticated', 'authenticated', 'hab200-old-owner@test.local', 'x',
    '{"full_name":"Original Owner"}'::jsonb, now(), now()
  ),
  (
    '20000000-0000-0000-0000-000000000012',
    '00000000-0000-0000-0000-000000000000',
    'authenticated', 'authenticated', 'hab200-new-owner@test.local', 'x',
    '{"full_name":"New Owner"}'::jsonb, now(), now()
  );
...
values
  (
    ...
    (select (payload #>> '{condominium,id}')::uuid from hab200_workspace),
    '20000000-0000-0000-0000-000000000011',
    'Original', 'Owner', 'hab200-old-owner@test.local', 'active',
    '20000000-0000-0000-0000-000000000001'
  ),
  (
    ...
    (select (payload #>> '{condominium,id}')::uuid from hab200_workspace),
    '20000000-0000-0000-0000-000000000012',
    'New', 'Owner', 'hab200-new-owner@test.local', 'active',
    '20000000-0000-0000-0000-000000000001'
  );
...
select hasnt_table_privilege(
  ...
  'INSERT',
  'authenticated users cannot insert governance snapshots directly'
);
...
select public.create_governance_proposal(
  ...
  'La elegibilidad de esta votación debe permanecer estable aunque cambie la propiedad.',
  'improvement',
  'one_per_unit',
  50,
  1000,
...
select is(
  ...
  'open',
  'opening a proposal captures eligibility and opens voting atomically'
);
...
select is(
  ...
  1::bigint,
  'one eligible unit produces one immutable snapshot entitlement'
);

select is(
  (
    select eligibility_count
    from public.governance_proposals
    where id = (select (proposal).id from hab200_proposal)
  ),
  1,
  'proposal stores the frozen eligible voting-entity count'
);

select is(
  (
    select eligibility_snapshot_source
    from public.governance_proposals
    where id = (select (proposal).id from hab200_proposal)
  ),
  'lifecycle',
  'normal opening marks the snapshot as lifecycle-captured'
);
...
select is(
  ...
  true,
  'original snapshotted owner keeps the original voting entitlement'
);
...
select is(
  ...
  1,
  'original voter still sees the snapshotted unit after ownership transfer'
);
...
select is(
  ...
  false,
  'new owner is not retroactively added to an already-open vote'
);
...
select throws_ok(
  ...
  'P0001',
  'snapshotted unit ownership required',
  'new owner cannot vote with ownership acquired after opening'
);
...
select is(
  ...
  1,
  'ownership transfer does not change the frozen quorum denominator'
);
...
select is(
  ...
  true,
  'quorum uses the frozen denominator after ownership changes'
);
...
select throws_ok(
  ...
  'P0001',
  'governance eligibility snapshot is immutable',
  'snapshot rows cannot be changed after capture'
);
...
select is(
  ...
  'closed',
  'snapshotted proposal can close normally'
);
...
select is(
  ...
  'approved',
  'certified winner can be formally approved'
);

select ok(
  (
    select certified_at is not null
    from public.governance_proposals
    where id = (select (proposal).id from hab200_proposal)
  ),
...
select throws_ok(
  ...
  'P0001',
  'governance decision certification is immutable',
