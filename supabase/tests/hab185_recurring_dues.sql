...
select is((select relrowsecurity from pg_class where oid = 'public.recurring_charge_runs'::regclass), true, 'recurring runs use RLS');
select ok(not has_table_privilege('authenticated', 'public.recurring_charge_runs', 'INSERT'), 'authenticated clients cannot insert recurring runs directly');
...
select lives_ok($$select public.schedule_recurring_charge_run((select id from public.recurring_charge_plans where name='Cuota ordinaria Torre A'),'2026-09')$$, 'scheduling the same period is idempotent');
select is((select count(*) from public.recurring_charge_runs where period='2026-09'), 1::bigint, 'idempotent scheduling creates one occurrence');
select throws_ok($$select public.post_recurring_charge_run((select id from public.recurring_charge_runs where period='2026-09'))$$, 'P0001', 'recurring charge run must be reviewed before posting', 'scheduled run cannot post without human review');

select lives_ok($$select public.prepare_recurring_charge_run((select id from public.recurring_charge_runs where period='2026-09'))$$, 'administrator prepares occurrence for review');
select is((select status::text from public.recurring_charge_runs where period='2026-09'), 'pending_review', 'prepared occurrence waits for approval');
select is((select jsonb_array_length(distribution_snapshot) from public.recurring_charge_runs where period='2026-09'), 2, 'building scope includes only its two active units');
select is((select sum((row->>'amount')::numeric) from public.recurring_charge_runs r, lateral jsonb_array_elements(r.distribution_snapshot) row where r.period='2026-09'), 100.01::numeric, 'deterministic cent allocation reconciles exactly to requested total');
select is((select total_amount from public.recurring_charge_runs where period='2026-09'), 100.01::numeric, 'review snapshot freezes requested total');
reset role;
select is((select count(*) from public.integration_outbox where event_type='finance.recurring_charge.pending_review'), 1::bigint, 'pending review emits one durable outbox event');
set local role authenticated;
select set_config('request.jwt.claim.sub', '00000000-0000-0000-0000-000000018501', true);

select lives_ok($$select public.post_recurring_charge_run((select id from public.recurring_charge_runs where period='2026-09'))$$, 'authorized administrator approves and posts reviewed occurrence');
select is((select status::text from public.recurring_charge_runs where period='2026-09'), 'posted', 'approved recurring occurrence becomes posted');
select is((select status::text from public.charge_batches where id=(select charge_batch_id from public.recurring_charge_runs where period='2026-09')), 'posted', 'recurring posting reuses immutable charge batch ledger path');
select is((select count(*) from public.receivable_items where charge_batch_id=(select charge_batch_id from public.recurring_charge_runs where period='2026-09')), 2::bigint, 'posting creates one receivable per scoped unit');
select is((select sum(original_amount) from public.receivable_items where charge_batch_id=(select charge_batch_id from public.recurring_charge_runs where period='2026-09')), 100.01::numeric, 'posted receivables reconcile to frozen occurrence total');
reset role;
...
select throws_ok($$select public.schedule_recurring_charge_run((select id from public.recurring_charge_plans where name='Cuota ordinaria Torre A'),'2026-11')$$, 'P0001', 'permission denied', 'board member cannot create financial ledger work without finance permission');
