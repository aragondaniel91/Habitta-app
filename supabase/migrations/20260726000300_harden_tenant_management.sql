...
alter table public.buildings
  add constraint buildings_id_condominium_unique unique (id, condominium_id);
alter table public.units drop constraint if exists units_building_id_fkey;
alter table public.units
  add constraint units_building_condominium_fkey
  foreign key (building_id, condominium_id)
  references public.buildings(id, condominium_id)
  on delete set null (building_id);

create or replace function public.can_manage_condominium_structure(target uuid)
returns boolean
language sql
stable
security definer
...
for all
using (public.can_manage_condominium_structure(buildings.condominium_id))
with check (public.can_manage_condominium_structure(buildings.condominium_id));
create policy unit_write on public.units
for all
using (public.can_manage_condominium_structure(units.condominium_id))
with check (public.can_manage_condominium_structure(units.condominium_id));

create or replace function public.create_condominium(target_organization_id uuid, condominium_name text)
...
begin
  ...
  values (target_organization_id, condominium_name, auth.uid())
  returning * into created;

  insert into public.condominium_memberships(condominium_id, user_id, role)
  values (created.id, auth.uid(), 'condominium_admin');
