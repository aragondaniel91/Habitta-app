create table public.structure_imports (
...
alter table public.structure_imports enable row level security;

create policy structure_imports_read on public.structure_imports
for select
using (public.can_manage_condominium_structure(condominium_id));

create function public.preview_structure_import(target uuid, rows jsonb)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  loop
    ...
    if unit_code_value = '' then
      ...
      issue := 'unit_code is duplicated in the file';
...
create function public.import_structure_csv(
  target uuid,
  rows jsonb,
  key text,
...
revoke execute on function public.preview_structure_import(uuid, jsonb) from public;
revoke execute on function public.import_structure_csv(uuid, jsonb, text, text) from public;
