-- HAB-419: preserve valid single-building topology throughout condominium purge.
--
-- The tenant purge previously attempted public.buildings before public.units because
-- tenant-scoped tables are visited alphabetically. For a single_building condominium,
-- deleting the building caused the units FK to SET NULL building_id, firing the HAB-209
-- topology trigger while zero configured buildings temporarily existed.
--
-- Buildings are now deferred from the generic purge loop. Once unit-scoped dependents
-- and units have been removed, the final condominium DELETE removes buildings through
-- the existing ON DELETE CASCADE. Triggers, RLS, append-only guards, authorization,
-- deletion tombstones and storage cleanup remain unchanged.
  ...
  loop
    ...
    for tenant_table in
      ...
      where n.nspname = 'public'
        ...
        -- HAB-419: buildings must outlive units during the purge. Deleting a building
        -- while units still reference it invokes ON DELETE SET NULL on units.building_id,
        -- which fires the HAB-209 topology guard and creates an invalid transient
        -- zero-building state for single_building condominiums. The final condominium
        -- delete cascades the now-unreferenced buildings after units are gone.
        and c.relname not in ('buildings', 'condominiums', 'condominium_deletion_jobs')
      order by c.relname
    loop
      begin
