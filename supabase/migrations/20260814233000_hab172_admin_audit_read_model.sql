-- HAB-172: normalized, read-only administrator audit feed.
-- Reuses immutable/module event history instead of creating a second competing audit log.
-- Sensitive module metadata is deliberately allowlisted; internal reasons, ballots, proof data,
-- message bodies, credentials and tokens are never returned by this read model.

create function public.can_read_admin_audit_log(target uuid)
returns boolean
language sql
stable
security definer
...
revoke execute on function public.can_read_admin_audit_log(uuid) from public;
grant execute on function public.can_read_admin_audit_log(uuid) to authenticated, service_role;

create index if not exists expense_events_condominium_audit_idx
  on public.expense_events (condominium_id, occurred_at desc, id desc);
create index if not exists governance_events_condominium_audit_idx
  on public.governance_events (condominium_id, occurred_at desc, id desc);
create index if not exists assembly_events_condominium_audit_idx
  on public.assembly_events (condominium_id, occurred_at desc, id desc);
