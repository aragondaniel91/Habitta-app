...
create function public.is_organization_owner(target uuid) returns boolean language sql stable security definer set search_path=public as $$
 select exists(select 1 from organization_memberships where organization_id=target and user_id=auth.uid() and role='organization_owner'); $$;
create function public.is_condominium_admin(target uuid) returns boolean language sql stable security definer set search_path=public as $$
 select exists(select 1 from condominium_memberships where condominium_id=target and user_id=auth.uid() and role in ('condominium_admin','accountant','assistant')); $$;
...
begin
 ...
 insert into organizations(name,created_by) values (organization_name,auth.uid()) returning * into org;
 insert into organization_memberships(organization_id,user_id,role) values(org.id,auth.uid(),'organization_owner');
 if condominium_name is not null then
  insert into condominiums(organization_id,name,created_by) values(org.id,condominium_name,auth.uid()) returning * into condo;
  insert into condominium_memberships(condominium_id,user_id,role) values(condo.id,auth.uid(),'condominium_admin');
 end if;
 return jsonb_build_object('organization',to_jsonb(org),'condominium',case when condo.id is null then null else to_jsonb(condo) end);
end; $$;
