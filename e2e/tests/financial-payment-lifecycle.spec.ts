...
type LedgerEntry = { direction: 'debit' | 'credit'; amount: string };
...
const balance = async (request: APIRequestContext, token: string) => {
  const response = await request.get(
    `${supabaseUrl}/rest/v1/receivable_ledger_entries?receivable_item_id=eq.${ids.receivableItem}&select=direction,amount`,
    {
