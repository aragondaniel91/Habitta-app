...
async function request(path, { method = 'GET', body } = {}) {
  ...
  if (!response.ok) {
    throw new Error(
      payload?.message ?? payload?.error_description ?? `Solicitud rechazada (${response.status}).`,
    );
  }
  return payload;
}
...
async function loadData() {
  ...
  try {
    ...
    const message =
      error instanceof Error ? error.message : 'No se pudo cargar el estado comercial.';
...
function adjustmentLabel(row) {
  ...
  if (row.adjustment_source === 'gift')
    return `Regalado · hasta ${formatDate(row.adjustment_effective_to)}`;
  if (row.adjustment_source === 'coupon')
    return `Cupón · hasta ${formatDate(row.adjustment_effective_to)}`;
  return '—';
}
...
function renderMetrics() {
  ...
  metricTrials.textContent = String(
    customers.filter((row) => row.subscription_status === 'trialing').length,
  );
  metricActive.textContent = String(
    customers.filter((row) => row.subscription_status === 'active').length,
  );
...
function priceLabel(row) {
  ...
  if (
    row.effective_period_amount !== null &&
    Number(row.effective_period_amount) !== Number(row.contracted_period_amount)
  ) {
...
function renderCustomers() {
  ...
  for (const row of rows) {
    ...
    status.append(statusBadge(row.subscription_status));
    tr.append(status);
    ...
    if (row.account_type !== 'customer') {
      ...
      if (row.subscription_status !== 'cancelled')
        actions.append(makeMini('Aplicar cupón', 'coupon', row, activeOffers().length === 0));
      if (['active', 'past_due'].includes(row.subscription_status))
        actions.append(makeMini('Regalar meses', 'gift', row));
      if (['trialing', 'suspended', 'past_due'].includes(row.subscription_status))
        actions.append(makeMini('Activar manual', 'activate', row));
    }
    tr.append(actions);
    customerBody.append(tr);
  }
...
function renderOffers() {
  ...
  if (!offers.length) {
    ...
    p.textContent =
      'Todavía no hay ofertas. Crea solo las que necesites para una venta o piloto real.';
    offerList.append(p);
    return;
  }
  for (const offer of offers) {
    ...
    head.append(
      code,
      badge(offer.active ? 'Activa' : 'Deshabilitada', offer.active ? 'green' : 'neutral'),
    );
    const p = document.createElement('p');
    p.textContent = offerDescription(offer);
...
function openAction(action, row) {
  ...
  if (action === 'trial') {
    ...
    dialogNotice.textContent =
      'No se cobrará hoy y auto-billing permanecerá apagado. Al vencer el trial, el acceso comercial falla cerrado hasta una activación explícita.';
    dialogFields.append(
      field(
        'Plan',
        selectControl(
    ...
    dialogCopy.textContent = `${row.organization_name} · ${row.condominium_name}`;
    dialogNotice.textContent =
      'El descuento será temporal. El precio contractual base no se modifica y HAB-424 no permite stacking.';
    ...
    dialogFields.append(
      ...
      field(
        ...
        dateControl(
          'startDate',
          inputDate(row.subscription_status === 'trialing' ? row.trial_ends_at : null),
        ),
      ),
    );
  } else if (action === 'gift') {
    ...
    dialogCopy.textContent = `${row.organization_name} · ${row.condominium_name}`;
    dialogNotice.textContent =
      'El período regalado queda como ajuste comercial a $0. No crea pagos, receivables ni movimientos contables.';
    dialogFields.append(
      field(
        'Meses',
        selectControl(
    ...
    dialogCopy.textContent = `${row.organization_name} · ${row.condominium_name}`;
    dialogNotice.textContent =
      'Esta acción confirma la suscripción como activa, pero NO habilita cobro automático ni registra un pago. Úsala solo cuando el acuerdo comercial haya sido confirmado fuera de Habitta.';
  }
...
offerForm.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    setStatus(
      offerStatus,
      error instanceof Error ? error.message : 'No se pudo crear la oferta.',
      'error',
    );
  } finally {
