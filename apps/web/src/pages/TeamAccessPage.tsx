...
export function TeamAccessPage({ condominiumId, condominiumName, session }: Props) {
  ...
  const pendingMemberName = pendingMemberAction
    ? (pendingMemberAction.member.full_name ?? pendingMemberAction.member.email)
    : '';
  const pendingMemberBusy = pendingMemberAction
    ? memberBusyId === pendingMemberAction.member.user_id
    : false;
              ...
              <div className="team-member-list">
                {data.members.map((member) => {
                  ...
                  return (
                    <article
                      className={
                        member.status === 'suspended' ? 'team-member--suspended' : undefined
                      }
                      key={member.user_id}
                    >
        ...
        <ConfirmDialog
          ...
          description={
            pendingMemberAction.action === 'remove'
              ...
              : `${pendingMemberName} perderá temporalmente el acceso operativo a ${condominiumName}. Podrás reactivarlo después.`
          }
          onCancel={() => !pendingMemberBusy && setPendingMemberAction(null)}
          onConfirm={() =>
